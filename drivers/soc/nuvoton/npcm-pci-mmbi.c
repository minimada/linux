...
#define NPCM_MAX_WIN_SIZE	16384

#define MAX_PCI_CH	8
#define DEFUALT_PCI_CH	0

#define BMBXSTAT		0x0
#define BMBXCTL			0x4
#define BMBXCMD			0x8
#define MMBI_PCI_HOST_RESET	BIT(31)
...
static void npcm_mmbi_enable_interrupt(struct npcm_pci_mmbi *priv, bool enable)
{
	u8 val;

	val = readl(priv->regs + BMBXCTL);
	if (enable)
		val |= BIT(priv->pci_ch);
	else
		val &= ~BIT(priv->pci_ch);
	writel(val, priv->regs + BMBXCTL);
}

static void raise_sci_interrupt(struct npcm_mmbi_channel *channel)
{
	struct npcm_pci_mmbi *priv = channel->priv;

	dev_dbg(priv->dev, "Raising PCI notify interrupt...\n");
	writel(BIT(priv->pci_ch), priv->regs + BMBXCMD);
}
...
static struct npcm_mmbi_protocol *file_npcm_pci_mmbi(struct file *file)
{
...
{
	struct npcm_mmbi_protocol *protocol = file_npcm_pci_mmbi(filp);
...
static const struct file_operations npcm_pci_mmbi_fops = {
	.owner = THIS_MODULE,
	.open = mmbi_open,
...
static int mmbi_channel_init(struct npcm_pci_mmbi *priv, u8 idx)
{
...
{
	struct npcm_pci_mmbi *priv = container_of(to_delayed_work(work),
		struct npcm_pci_mmbi, work);
...
static irqreturn_t npcm_pci_mmbi_irq(int irq, void *arg)
{
	struct npcm_pci_mmbi *priv = arg;
	int val;

	dev_dbg(priv->dev, "MMBI IRQ Status: 0x%x\n",
		readl(priv->regs + BMBXSTAT));

	val = readl(priv->regs + BMBXSTAT);
	/*we should check what we need to do if the HOS is reset */
	/* Clear interrupt status */
	writel(BIT(priv->pci_ch), priv->regs + BMBXSTAT);

	if (val & MMBI_PCI_HOST_RESET) {
		dev_dbg(priv->dev, "Host reset occur\n");
		writel(MMBI_PCI_HOST_RESET, priv->regs + BMBXSTAT);
	}
...
static int npcm_pci_mmbi_probe(struct platform_device *pdev)
{
	struct npcm_pci_mmbi *priv;
	struct device_node *node;
	...
	priv = devm_kzalloc(&pdev->dev, sizeof(struct npcm_pci_mmbi),
			    GFP_KERNEL);
	if (!priv)
		return -ENOMEM;
	...
	if (of_property_read_bool(priv->dev->of_node, "use-interrupt")) {
		if (!of_property_read_u32(priv->dev->of_node, "pci-channel", &val))
			priv->pci_ch = val >= MAX_PCI_CH ? DEFUALT_PCI_CH : val;
		else
			priv->pci_ch = DEFUALT_PCI_CH;

		dev_info(priv->dev, "use interrupt, channel->%d\n",priv->pci_ch);
		/* Enable IRQ */
		...
		rc = devm_request_irq(priv->dev, priv->irq, npcm_pci_mmbi_irq,
	...
	dev_info(priv->dev, "MMBI: npcm MMBI PCI driver loaded successfully\n");

	return 0;
}

static int npcm_pci_mmbi_remove(struct platform_device *pdev)
{
	struct npcm_pci_mmbi *priv = dev_get_drvdata(&pdev->dev);
	int i, j;
...
static const struct of_device_id npcm_pci_mmbi_match[] = {
	{ .compatible = "nuvoton,npcm845-pci-mmbi" },
	{}
};
MODULE_DEVICE_TABLE(of, npcm_pci_mmbi_match);

static struct platform_driver npcm_pci_mmbi_driver = {
	.driver = {
		.name           = DEVICE_NAME,
		.of_match_table = npcm_pci_mmbi_match,
	},
	.probe  = npcm_pci_mmbi_probe,
	.remove = npcm_pci_mmbi_remove,
};
module_platform_driver(npcm_pci_mmbi_driver);

MODULE_AUTHOR("AppaRao Puli <apparao.puli@intel.com>");
MODULE_AUTHOR("Tomer Maimon <tomer.maimon@nuvoton.com>");
MODULE_AUTHOR("Stanley Chu <yschu@nuvoton.com>");
MODULE_AUTHOR("Ban Feng <kcfeng0@nuvoton.com>");
MODULE_DESCRIPTION("Nuvoton NPCM pci MMBI Driver");
