...
/*
 * Nuvoton NPCM ESPI flash Controller driver
 *
 * Copyright (C) 2024 Nuvoton Technologies
 */
...
enum ESPI_FLASH_BLOCK_ERASE_SIZE {
...
static bool npcm_espi_check_tag_ovr(struct npcm_espi_flash *priv,
				    u32 prtr_baddr,
				    u8 command, uint8_t tag, int idx)
{
	u16 frgn_wpr_tovr, frgn_rpr_tovr;
...
static bool npcm_espi_check_addr(struct npcm_espi_flash *priv, uint8_t command,
				 u8 tag, uint32_t addr)
{
	u32 prtr_haddr, high_addr, prtr_baddr, base_addr;
	bool ret = true;
	int i;

	for (i = 0; i < ESPI_FLASH_SAF_PROT_MEM_NUM; i++) {
		regmap_read(priv->espi_regmap,
			    ESPI_FLASH_PRTR_HADDR(i), &prtr_haddr);
		high_addr = FIELD_GET(FLASH_PRTR_HADDR_MASK, prtr_haddr);
		if (high_addr != 0) {
			regmap_read(priv->espi_regmap,
				    ESPI_FLASH_PRTR_BADDR(i), &prtr_baddr);
...
static irqreturn_t npcm_espi_flash_irq_handler(int irq, void *arg)
{
	...
	if (val & ESPISTS_FLASHRX) {
		...
		trace_npcm_espi_flash(command, address, length);
